...
type sm3Digest struct {
	v         [sm3DigestLength / 4]uint32
	inWords   [sm3BlockSize / 4]uint32
	xOff      int32
	w         [68]uint32
	xBuf      [4]byte
	xBufOff   int32
...
func (digest *sm3Digest) checkSum() [sm3DigestLength]byte {
	digest.finish()
	var out [sm3DigestLength]byte
	for i := 0; i < len(digest.v); i++ {
...
func (digest *sm3Digest) processBlock() {
	...
	for j := 16; j < 68; j++ {
		...
		wj13 := digest.w[j-13]
		r7 := (wj13 << 7) | (wj13 >> (32 - 7))
		digest.w[j] = sm3P1(digest.w[j-16]^digest.w[j-9]^r15) ^ r7 ^ digest.w[j-6]
	}
	...
	for j := 0; j < 16; j++ {
		...
		Wj := digest.w[j]
		W1j := Wj ^ digest.w[j+4]
		TT1 := sm3FF0(A, B, C) + D + SS2 + W1j
		TT2 := sm3GG0(E, F, G) + H + SS1 + Wj
		D = C
		C = (B << 9) | (B >> (32 - 9))
		B = A
		A = TT1
	...
	for j := 16; j < 64; j++ {
		...
		Wj := digest.w[j]
		W1j := Wj ^ digest.w[j+4]
		TT1 := sm3FF1(A, B, C) + D + SS2 + W1j
		TT2 := sm3GG1(E, F, G) + H + SS1 + Wj
		D = C
		C = (B << 9) | (B >> (32 - 9))
		B = A
		A = TT1
...
func (digest *sm3Digest) processLength(bitLength int64) {
	if digest.xOff > (sm3BlockSize/4 - 2) {
		digest.inWords[digest.xOff] = 0
		digest.xOff++
		digest.processBlock()
	}
	for ; digest.xOff < (sm3BlockSize/4 - 2); digest.xOff++ {
		digest.inWords[digest.xOff] = 0
	}
...
func sm3Sum(data []byte) [sm3DigestLength]byte {
	var d sm3Digest
	d.Reset()
	d.Write(data)
	return d.checkSum()
